...
describe('perframe', function()
    ...
    it('should be concerningly pedantic about run order', function()
        ...
        xero.perframe {3, 4, function(beat)
            if beat == 4 then
                assert.equal(2, order) order = 3
            elseif beat == 6 then
                assert.equal(6, order) order = 7
            else
                error('bad beat')
            end
        end}
        xero.perframe {1, 4, function(beat)
            if beat == 2 then
                assert.equal(0, order) order = 1
            elseif beat == 4 then
                assert.equal(3, order) order = 4
            else
                error('bad beat')
            end
        end}
        xero.perframe {1, 6, function(beat)
            if beat == 2 then
                assert.equal(1, order) order = 2
            elseif beat == 4 then
                assert.equal(5, order) order = 6
            elseif beat == 6 then
                assert.equal(8, order) order = 9
            else
                error('bad beat')
            end
        end, defer = true}
        xero.perframe {3, 4, function(beat)
            if beat == 4 then
                assert.equal(order, 4) order = 5
            elseif beat == 6 then
                assert.equal(order, 7) order = 8
            else
                error('bad beat')
            end
        end}
